...
class Instructor:
    ...
    def __init__(self, opt):
        self.opt = opt
        with open('his', 'a') as f:
            f.write(vars(opt)['model_name']+'\n')
    ...
    def _train(self, criterion, optimizer, max_test_acc_overall=0):
        ...
        max_f1 = 0
        global_step = 0
        lossL, accL, test_accL = [], [], []
        for epoch in range(self.opt.num_epoch):
            print('>' * 50)
            print('epoch:', epoch)
            n_correct, n_total = 0, 0
            LL2, accL2, test_accL2 = [], [], []
            for i_batch, sample_batched in enumerate(self.train_dataloader):
                ...
                if global_step % self.opt.log_step == 0:
                    ...
                    accL2.append(train_acc)
                    test_accL2.append(test_acc)
                    LL2.append(loss.item())
            ...
            lossL.append(sum(LL2)/len(LL2))
            # print(accL2)
            accL.append(sum(accL2)/len(accL2))
            test_accL.append(max(test_accL2))
        return max_test_acc, max_f1, lossL, accL, test_accL
    ...
    def run(self, repeats=1):
        ...
        for i in range(repeats):
            print('repeat:', i)
            self._reset_params()
            max_test_acc, max_f1, lossL, accL, test_accL = self._train(criterion, optimizer, max_test_acc_overall)
            print('max_test_acc: {0}, max_f1: {1}'.format(max_test_acc, max_f1))
            max_test_acc_overall = max(max_test_acc, max_test_acc_overall)
            max_f1_overall = max(max_f1, max_f1_overall)
            print('#' * 50)
            with open('his', 'a') as f:
                f.write('lossL'+str(lossL)+'\n')
                f.write('taccL'+ str(accL)+'\n')
                f.write('test_accL'+ str(test_accL)+'\n')
...
def main():
    ...
    input_colses = {
        ...
        'tcnn': ['text', 'aspect', 'position'],
        'gcae':['text', 'aspect'],
        'pgcae':['text', 'aspect', 'position'],
        'kcnn': ['text']
    }
    ...
    parser.add_argument('--learning_rate', default=0.001, type=float) # 0.005 更好
    parser.add_argument('--dropout', default=0.3, type=float)
    parser.add_argument('--l2reg', default=1e-5, type=float)
    parser.add_argument('--num_epoch', default=15, type=int) # 20
